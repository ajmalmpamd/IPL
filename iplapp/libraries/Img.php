...
class img {
    ...
    public function rimg($source, $params, $no_cache = FALSE) {
        ...
        if (!empty($params['data_id'])) {
            $extra_parameters .= " data_id=\"{$params['data_id']}\" ";
        }
...
//            return "<img src=\"{$this->ci->config->config['img']['base_url']}\\" . basename($dst['file']) . "\" width=\"{$dst['width']}\" height=\"{$dst['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
            return "<img src=\"{$this->ci->config->config['img']['base_url']}\\" . basename($dst['file']) . "\"  alt=\"{$params['alt']}\" {$extra_parameters}/>";
        }

        // if file exists - return img info
        if (file_exists($dst['file']) AND $no_cache != TRUE) {
//            return "<img src=\"{$this->ci->config->config['img']['base_url']}{$dir}/" . basename($dst['file']) . "\" width=\"{$dst['width']}\" height=\"{$dst['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
            return "<img src=\"{$this->ci->config->config['img']['base_url']}{$dir}/" . basename($dst['file']) . "\"  alt=\"{$params['alt']}\" {$extra_parameters}/>";
        }
...
//        return "<img src=\"{$this->ci->config->config['img']['base_url']}{$dir}/" . basename($dst['file']) . "\" width=\"{$dst['width']}\" height=\"{$dst['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
    ...
    public function aimg($source, $params, $no_cache = FALSE) {

        $same_image_source = trim($source);
        ...
        if (!empty($params['width'])) {
            ...
            $dst['width'] = intval($params['height']) * ($src['width'] / $src['height']);
        }
        if (!empty($params['height'])) {
            $dst['height'] = intval($params['height']);
            // $dst['height'] = intval(400);
            // $dst['height'] = intval($params['width'])*($src['height']/$src['width']);
        } else {
            $dst['height'] = intval($params['width']) * ($src['height'] / $src['width']);
        }
        //$dst['height'] =intval(500);
        //$params['height']=intval(500);
        //$dst['height'] = intval($params['width'])*($src['height']/$src['width']);
        //p($src);
        // if alt is empty, setup file name - bad idea ;)
        if (empty($params['alt'])) {
...
//            $source1 = 'http://drinn.in' . preg_replace('/\s\s+/', '.', $source);
              $source1 = 'http://drinn.in' .  $source;                   
            
            return "<img src=" . $source1 . "  alt=\"{$params['alt']}\" {$extra_parameters}/>";
        }
